  ...
  return (
    <main className="container-page">
      <section className="print-card rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <h1 className="mb-6 text-2xl font-bold">{COPY.start.heading}</h1>
        <p className="mb-6 text-sm text-slate-600">{COPY.start.intro}</p>
              ...
              {COPY.start.ministryOptions.map((option) => (
                <label key={option} className="flex cursor-pointer items-center gap-2 rounded-md border border-slate-300 px-3 py-2 text-sm">
                  <input
                    type="radio"
                    name="ministry"
                    checked={ministry === option}
                    onChange={() => setMinistry(option)}
                  />
                  {option}
                </label>
              ))}
              ...
              {COPY.start.churchSizeOptions.map((option) => (
                <label key={option} className="flex cursor-pointer items-center gap-2 rounded-md border border-slate-300 px-3 py-2 text-sm">
                  <input
                    type="radio"
                    name="churchSize"
                    checked={churchSize === option}
                    onChange={() => setChurchSize(option)}
                  />
                  {option}
                </label>
              ))}
